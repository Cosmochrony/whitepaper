...
coarse-grained dynamics}.
    ...
    \begin{figure}[t]
      \centering
      \includegraphics[width=0.78\linewidth]{D-appendix-technical/fig_D4_saturation_fraction_compare}
  ...
  \paragraph{Interpretation and limitations.}
    ...
    the coarse-grained micro-dynamics in projectable regimes, as indicated by a small
    normalized residual $\varepsilon=\mathcal{O}(10^{-2})$ for both smooth and rough runs
    across multiple lattice resolutions.

    However, the residual $\varepsilon$ is not by itself a sufficient criterion for
    projectability.
    In fully saturated configurations, where $S_i>1$ on (nearly) all lattice sites,
    the bounded relaxation term vanishes ($R_i=0$) and the dynamics becomes effectively
    frozen.
    In this case, $\partial_t\chi_{\mathrm{eff}}\approx 0$ and the residual
    $\varepsilon$ becomes trivially small, even though no meaningful effective geometric
    description exists.
    Such saturated regimes therefore constitute explicit non-projectable failure modes,
    which must be identified using an independent indicator.
