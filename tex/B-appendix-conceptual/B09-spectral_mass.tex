\subsection{Spectral Characterization of Mass and the Secondary Role of \(V(\chi)\)}
  ...
  \paragraph{Origin of the effective potential $V(\chi)$.}
    The characterization of $V(\chi)$ as a secondary quantity does not imply that it is
    arbitrary or postulated independently.
    Rather, $V(\chi)$ should be understood as an effective, coarse-grained descriptor
    of the nonlinear response of the $\chi$ relaxation dynamics in the vicinity of a
    localized configuration.

    At the fundamental level, the dynamics of $\chi$ are governed by the bounded
    relaxation operator and its spectral structure.
    When this dynamics is projected onto a reduced functional subspace associated
    with a stable solitonic configuration, nonlinear self-consistency constraints give rise
    to an effective local restoring structure.
    In this reduced description, these nonlinear constraints may be summarized by an
    effective potential $V(\chi)$.

    The form of $V(\chi)$ therefore encodes neither an independent interaction nor a
    fundamental energy density.
    It reflects the local stiffness of the relaxation dynamics induced by the global
    ...
    choices, while leaving invariant the underlying spectral origin of mass and stability.

    A first-principles derivation of $V(\chi)$ from the full nonlinear relaxation dynamics
    remains an open problem, but its existence as an effective descriptor follows
    generically from the projection of bounded nonlinear dynamics onto localized modes.
