\section{Relation to Quantum Formalism}
  ...
  \subsection{Status of the Wavefunction}\label{subsec:status-of-the-wavefunction}
    ...
    Instead, $\chi$ constitutes a real geometric substratum defined on spacetime, from which effective quantum
    wavefunctions emerge as coarse-grained descriptions of localized excitations.
    The quantum wavefunction is thus interpreted as a derived object encoding the statistical behavior of
    $\chi$-mediated structures rather than as a fundamental entity.
    ...
    The probability density $|\psi|^2$ reflects the spatial distribution of $\chi$-curvature, while energy quantization
    $E_n \propto -1/n^2$ arises from the discrete topological winding numbers permitted by the boundary conditions at $r \to 0$
    and $r \to \infty$.
  ...
  \subsection{Emergence of the Schrödinger Equation from $\chi$ Fluctuations}
    ...
    around stable solitonic configurations.
    In this section, we provide an explicit and standard derivation of the Schr\"odinger equation as the
    non-relativistic limit of such fluctuations, making all approximations transparent.
    ...
    \subsubsection{Non-relativistic limit: Klein--Gordon $\rightarrow$ Schr\"odinger}
      ...
      We therefore use the standard ansatz
      \begin{equation}
      ...
      \paragraph{Cosmochrony-specific content.}
        ...
        relativistic scalar excitation.
        In Cosmochrony, the program is to derive (i) the effective mass $m$ from soliton energetics and (ii) the form of
        $V(x)$ from interaction-induced deformations of $\chi_{\mathrm{sol}}(x)$.
    ...
    \subsubsection{Interpretation}
      ...
      around a solitonic particle state.
      The Schr\"odinger equation thus appears as the universal non-relativistic limit of localized $\chi$ excitations,
      rather than as a fundamental postulate.
      ...
      deformations of the $\chi$ background.
      A detailed derivation of these quantities from the full $\chi$ action is left for future work.

      The effective Schr\"odinger description is therefore valid only within the regime of weak,
      coherent $\chi$-field fluctuations.
      Interactions that induce strong local deformations of $\chi$ drive the system outside this
      linearized regime, leading to decoherence without contradicting the emergence of
      Schr\"odinger dynamics where its underlying assumptions hold.
  ...
  \subsection{Origin of Quantization}
    ...
    The relation $E = h \nu$ emerges as a geometric proportionality between oscillation frequency and curvature energy
    stored in localized $\chi$ configurations.
  ...
  \subsection{Measurement and the Born Rule}
    ...
    specific topological sector of the $\chi$-field.
    For example, a photon detector absorbs energy by fixing a localized crest in $\chi$, effectively ``cutting'' the extended
    wave configuration and collapsing it to a particle-like excitation.
    The Born rule $P \propto |\psi|^2$ then follows from the statistical distribution of $\chi$-fluctuations that satisfy the
    detector's constraints, without requiring an intrinsic probabilistic postulate.
  ...
  \subsection{Spin and Statistics}
    ...
    rotation group.
    The necessity of $4\pi$ rotations for fermions is usually accepted as a mathematical fact without deeper geometric
    explanation.
  ...
  \subsection{Orbital Geometry as Probabilistic Visibility of Underlying $\chi$ Fluctuations}
    ...
    Atomic orbitals are commonly visualized as isosurfaces of the probability density $|\psi|^2$
    exceeding a fixed threshold.
    Whether such visualizations appear as connected regions or as multiple disjoint lobes, they
    represent probabilistic structures rather than spatially occupied material zones.

    In the Cosmochrony framework, these probability distributions are not taken as primitive.
    They arise from stationary fluctuation modes of the fundamental $\chi$ field, which provides
    the underlying dynamical substrate.
    Orbital shapes therefore encode the regions where fluctuations of $\chi$ are most likely to
    give rise to detectable material manifestations, without implying permanent occupation of
    those regions.

    From this perspective, orbital visualizations capture only the visible portion of an
    underlying four-dimensional wave-like configuration.
    The probabilistic character of the orbitals reflects the fluctuating nature of this
    configuration, while their geometric stability reflects the existence of stationary modes.

    When present, the disjoint appearance of orbital lobes offers a particularly suggestive
    geometric indication of this interpretation.
    Such disjointness naturally arises when only certain crest regions of the underlying
    fluctuation pattern exceed the threshold for manifestation, while intermediate regions
    remain below detectability.
    Importantly, this effect concerns the visibility of probability density rather than the
    spatial distribution of material substance.

    While the detailed manifestations of orbital probability densities are influenced by
    stochastic fluctuations of the $\chi$ field, the overall orbital geometry remains fixed by
    the underlying topological configuration of $\chi$ associated with the corresponding
    excitation.
    In this sense, orbital shapes encode invariant structural features of $\chi$, such as nodal
    surfaces and symmetry properties, while their local intensity and apparent extent reflect
    the fluctuating nature of the field.

    This distinction parallels the behavior observed in interference experiments.
    In a double-slit setup, the interference pattern is determined by the underlying wave
    structure and remains invariant, even though individual detection events occur at random
    locations.
    Similarly, atomic orbitals correspond to stable probabilistic patterns imposed by the
    topology of $\chi$, modulated by stochastic fluctuations that govern the distribution of
    individual manifestations.

    A formal mathematical discussion of threshold visibility, level sets, and projection-induced
    fragmentation is provided in Appendix~\ref{app:level_sets_orbitals}.
